...
// https://github.com/polkadot-js/api/pull/5752/files#diff-ed559e84e01ac9b0166c3e3cba78445a16093a155349ca7b889400331fa2c26cR53
// https://github.com/polkadot-js/api/issues/5846

add_ah: function(a,CUR) { // добавляем в запрос ещё кое-какие нужные параметры, если assethub
    if(DOT.nodes[CUR].asset_id) {
	console.log('asset_id='+DOT.nodes[CUR].asset_id);
	...
	// vovke: This is a temporary solution, wasn't able to find a better way to solve it
        a.assetId = { parents: 0, interior: { X2: [{ palletInstance: 50 }, { generalIndex: DOT.nodes[CUR].asset_id }] } };
	// и это теперь тоже нужно для кошельков
	a.withSignedTransaction = true;
    }
    return a;
},
...
daemon_get_info: async function() {
    ...
    try {
	    ...
	    // разрешенные Магазином:
	    var m2 = DOT.cx.currences.replace(/,/g,' ').split(' ');

            console.log('allowed by store: '+m2.join(', '));
            console.log('allowed by daemon: '+m1.join(', '));
	    ...
	    // попробовать выбрать тот, что был сохранен
	    var last = DOT.f_read('kalatori_CUR');
	    if(last && last.length && last.length>0 && DOT.nodes[last]) DOT.CUR = last;
...
//    API_last: {    },
	...
	console.log(`DOT.Transfer to:${to} price:${price}_${CUR}`);
	console.log(`DOT.add_ah: `,DOT.add_ah({signer: injector.signer},CUR));

//	var tt = await DOT.Transfer(to, price, CUR);
//	console.log(`DOT.Transfer result: `,tt);
//	await tt
// .signAndSend(SENDER,
//
...
//     } catch(err) { console.log('Wallets crash: '+err);	return false; }
	...
	if(N.asset_id) { // в случае assetHub добавляем невыразимой мистической хуйни от шамана Габышева и Сёко Асахара
	    /*
	    // Не добавляем, потому что после смены метадаты всё перестало работать
	    a.signedExtensions = {
